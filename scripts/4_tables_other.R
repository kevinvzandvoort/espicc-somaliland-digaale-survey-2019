...
table2_travel = svyMean2(~contacts_total+home_contacts+school_or_work_contacts+other_contacts+contacts_other_median+
...
table2_travel[, variable := factor(variable, c("contacts_total", "home_contacts", "school_or_work_contacts",
...
table3_contacts = svyMean2(~travel_0_5km+travel_5_10km+travel_10km_gt, contact_data_frequency_ps, na.rm=T, digits=1,
                           multiply = 100) %>% .[!option %in% c("don’t know", "refuse to answer")]

table3_contacts[, ci95_low := pmax(0, ci95_low)]
table3_contacts[, mean_label := sprintf("%s%%", mean)]
table3_contacts[, confint := sprintf("%s - %s", ci95_low, ci95_high)]
table3_contacts[, variable := factor(variable, c("travel_0_5km", "travel_5_10km", "travel_10km_gt"),
                                     c("< 5km", "5 - 10km", "> 10km"))]
...
#' - This is a sensitivity analysis of estimates in table 2
