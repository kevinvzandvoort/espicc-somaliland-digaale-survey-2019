...
contact_data_full[, c("id", "contacts_other_est", "pw")] %>%
  merge(participant_data_design_ps$variables[, c("occupation", "household_size", "contactor_id")], by.x="id", by.y="contactor_id") %>%
  .[, c("part_id", "sample_weight") := .(id, pw)] %>%
  .[, c("part_id", "sample_weight", "household_size", "contacts_other_est")] %>%
  .[, country := "Somaliland"] %>% unique() %>%
  fwrite("./zenodo_socialmixr/espicc_somaliland_digaale_participant_extra.csv")

#' Create contact_common.csv file
contact_data_contactees = contact_data_full %>% setorder(id) %>% .[, contactee_id := .I]
contact_common = contact_data_contactees %>% copy %>% setorder(id) %>%
  .[, c("part_id", "cont_id", "cnt_age_exact", "cnt_age_est_min", "cnt_age_est_max", "cnt_gender",
      ...
      .(id, contactee_id, contact_age, NA_integer_, NA_integer_, contact_sex,
...
contact_data_full[, c("id", "contacts_day", "contacts_date")] %>%
  unique %>%
...
n_population_size_fpc %>% merge(age_groups_60, by.x="age_group_60", by.y="name") %>%
